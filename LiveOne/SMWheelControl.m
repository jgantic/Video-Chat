...
- (void)drawWheel
{
    ...
    for (int i = 0; i < numberOfSlices; i++) {
        ...
        for (UIView *j in sliceView.subviews) {
            if([j isKindOfClass:[UIImageView class]]){
                UIImageView *image = (UIImageView *)j;
                if (image.tag==111) {
                    image.transform = CGAffineTransformMakeRotation(2.4-(angleSize * i + snappingAngle));
                }
            }
        }
...
- (BOOL)continueTrackingWithTouch:(UITouch*)touch withEvent:(UIEvent*)event
{
    ...
    for (UIView *i in self.sliceContainer.subviews) {
        if([i isKindOfClass:[UIView class]]){
            ...
            for (UIView *j in view.subviews) {
                if([j isKindOfClass:[UIImageView class]]){
                    UIImageView *image = (UIImageView *)j;
                    if (image.tag==111) {
                        image.transform =  CGAffineTransformRotate(image.transform,-angleDelta);
...
{
    if (_status == SMWheelControlStatusIdle && touch.tapCount > 0) {
            ...
            if([i isKindOfClass:[UIView class]]){
                ...
                for (UIView *j in view.subviews) {
                    if([j isKindOfClass:[UIImageView class]]){
                        UIImageView *image = (UIImageView *)j;
                        if (image.tag==111) {
                            image.transform =  CGAffineTransformRotate(image.transform, angle);
...
- (void)didReceiveTapAtAngle:(CGFloat)angle
{
    if ([self.delegate respondsToSelector:@selector(wheel:didTapOnSliceAtIndex:)]) {
        ...
        CGFloat radiansPerSlice = 2.0 * M_PI / numberOfSlices;
        CGFloat snappingAngle = [self.dataSource respondsToSelector:@selector(snappingAngleForWheel:)] ? [self.dataSource snappingAngleForWheel:self] : 0.0;
        NSUInteger index = (lroundf((angle + snappingAngle - currentAngle) / radiansPerSlice) + numberOfSlices) % numberOfSlices;
