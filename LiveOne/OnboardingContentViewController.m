...
static CGFloat const kDefaultImageViewSize = 120;
static CGFloat const kDefaultImageViewHeightSize = 181;
...
- (instancetype)initWithTitle:(NSString *)title body:(NSString *)body image:(UIImage *)image videoURL:(NSURL *)videoURL buttonText:(NSString *)buttonText actionBlock:(action_callback)actionBlock {
    ...
    if (UIDeviceOrientationIsPortrait([UIDevice currentDevice].orientation)) {
        self.iconWidth =  2*(self.view.bounds.size.width/2);
    } else {
        self.iconWidth =  2*(self.view.bounds.size.width/2);
    }
    self.iconHeight = (self.view.bounds.size.height/2)-30;
