...
- (void) changeCenterImage:(UIImage*) centerImage {
    [_pathCenterButton setImage:centerImage forState:UIControlStateNormal];
}


- (void) changeCenterHighlightedImage:(UIImage*) centerImage {
    [_pathCenterButton setImage:self.centerHighlightedImage forState:UIControlStateHighlighted];
}
...
- (void)centerButtonTapped {
    ...
    return self.isBloom? [self pathCenterButtonFold] : [self pathCenterButtonBloom];
}
...
- (void)pathCenterButtonBloom {
    ...
    //
    CGFloat itemGapAngel = self.bloomAngel / (self.itemButtons.count - 1) ;
    CGFloat currentAngel = ((180.0f - self. bloomAngel)/2.0f+itemGapAngel)*_bloomCount;
    ...
    for (int i = 0; i < self.itemButtons.count; i++) {
        ...
        [self.bottomView insertSubview:pathItemButton belowSubview:self.pathCenterButton];
...
- (void)itemButtonTapped:(DCPathItemButton *)itemButton {
    ...
    if ([_delegate respondsToSelector:@selector(pathButton:clickItemButtonAtIndex:)]) {
        ...
        selectedButton.transform =  CGAffineTransformMakeScale(1, 1);
