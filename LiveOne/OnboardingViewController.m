...
- (void)viewWillLayoutSubviews {
    ...
    self.pageControl.frame = CGRectMake(0, CGRectGetMaxY(self.view.frame)-25.0, self.view.frame.size.width, kPageControlHeight);
    

    self.nextButton.frame = CGRectMake(self.view.frame.size.width/2+130, self.view.bounds.size.height/2-8, 18.0, 31.0);
    
    
    self.prevButton.frame = CGRectMake(self.view.frame.size.width/2-150, self.view.bounds.size.height/2-8, 18.0, 31.0);
...
- (void)generateView {
    ...
    if (self.shouldMaskBackground) {
        ...
        backgroundMaskView.backgroundColor = [UIColor colorWithWhite:0.0 alpha:0.0];
    ...
    [self.nextButton setImage:[UIImage imageNamed:@"arrow-right"]
                               forState:UIControlStateNormal];
    
    
    self.nextButton.contentMode = UIViewContentModeScaleAspectFit;
    [self.view addSubview:self.nextButton];
    
    [self.nextButton addTarget:self action:@selector(handleNextButtonPressed) forControlEvents:UIControlEventTouchUpInside];
    
    
    self.prevButton = [UIButton buttonWithType:UIButtonTypeCustom];
    
    [self.prevButton setImage:[UIImage imageNamed:@"arrow-left"]
                     forState:UIControlStateNormal];
    
    self.prevButton.contentMode = UIViewContentModeScaleAspectFit;
    [self.view addSubview:self.prevButton];
    
    [self.prevButton addTarget:self action:@selector(handlePrevButtonPressed) forControlEvents:UIControlEventTouchUpInside];
...
- (void)moveNextPage {
    ...
    NSLog(@"--current page--%lu", (unsigned long)indexOfNextPage);
...
- (void)movePrevPage {
    ...
    NSUInteger indexOfNextPage = [self.viewControllers indexOfObject:_currentPage] - 1;
     ...
     NSLog(@"--current page1--%lu", (unsigned long)indexOfNextPage);
    
    if (indexOfNextPage < self.viewControllers.count) {
        [self.pageVC setViewControllers:@[self.viewControllers[indexOfNextPage]] direction:UIPageViewControllerNavigationDirectionReverse animated:YES completion:nil];
        [self.pageControl setCurrentPage:indexOfNextPage];
    }
}
